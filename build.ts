...
const fencedCodeBlockDefinition = (name: string, identifiers: string[], sourceScope: string | string[], language: string | undefined, additionalContentName: string[] | undefined): string => {
...
const indent = (count: number, text: string): string => {
    const indentStr = new Array(count + 1).join('  ');
...
const buildGrammar = (): void => {
    let text = fs.readFileSync(path.join(import.meta.dirname, 'markdown.tmLanguage.base.yaml'), "utf8");
